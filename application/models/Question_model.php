...
public function	get_list_of_answered_question()
{
	...
	//$query="SELECT UP.user_name,Q.q_id, Q.q_title, GROUP_CONCAT(T.name) as tag_name, Q.no_of_answer, Q.no_of_likes, Q.created_on  FROM  answer A JOIN question Q ON A.q_id=Q.q_id JOIN question_tag QT ON QT.q_id=Q.q_id JOIN tags T ON T.tag_id=QT.tag_id JOIN user_profile UP ON Q.u_id=UP.u_id and answer.u_id='"$u_id"' GROUP BY Q.q_id, Q.q_title, Q.no_of_answer, Q.no_of_answer, Q.no_of_likes, Q.created_on ORDER BY Q.q_id DESC";//

				//LIMIT '".$data['from']."',10";

	$query = "select question.q_id,question.u_id,question.q_data,question.q_title,question.no_of_answer,question.no_of_likes,question.created_on,question.last_modified,user_profile.user_name ,group_concat(tags.name)from answer,question ,user_profile,tags where answer.u_id = '".$u_id."' and answer.q_id = question.q_id and question.u_id=user_profile.u_id and question.q_id = question_tag.q_id and question_tag.tad_id = tags.tag_id";
	//$execute = $this->db->query($query)->result_array();
...
public function get_list_of_question_asked()
{
	// $userdata = $_SESSION['user_data'];
	// $u_id = $user_data['u_id'];
	$u_id=19;

	$query = "select *  from question where u_id = '".$u_id."'";
	$execute = $this->db->query($query);
	//$row = $execute->row();
	
	// $user_name = $row_username->user_name;
	// array_push($row,'user_name'=>$user_name);
